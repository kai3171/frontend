<template>
  <div class="upload-box" v-if="!dataUploading">
    <b-container fluid>
      ...
      <b-alert v-model="errUnknownShow" variant="danger" show dismissible>Unknown error,please check your data</b-alert>
      <b-row class="my-1">
        <b-col sm="8">
          <b-container>
            <b-row class="my-2">
      ...
      <b-row><b-col>
        <b-button id="load" size="lg" variant="outline-primary"  @click="upload" v-if="!dataUploading">Upload Dataset</b-button>
        <b-button id="loading" size="lg" variant="danger" v-if="dataUploading" >Uploading</b-button></b-col>
      </b-row>
      <b-row></b-row>
    </b-container>
  </div>
  <div class="waittBox" v-else>
    <b-container fluid>
      <b-row class="my-1">
        <b-col>
          <h1>Your data have been uploaded,we are checking your data,please keep this page opening</h1>
        </b-col>
      </b-row>
      <hr />
      ...
      <b-row class="my-1">
        ...
        <b-col sm="3"><label class="labelFont">{{ wait_id }}</label></b-col>
      </b-row>
      <b-row class="my-1">
        <b-col sm="2"><label class="labelFont">Your name: </label></b-col>
        <b-col sm="3"><label class="labelFont">{{ wait_name }}</label></b-col>
      </b-row>
      <b-row class="my-1">
        <b-col sm="2"><label class="labelFont">Your email: </label></b-col>
        <b-col sm="3"><label class="labelFont">{{ wait_email }}</label></b-col>
      </b-row>
      <b-row class="my-1">
        <b-col sm="2"><label class="labelFont">Your Data: </label></b-col>
        <b-col sm="9"><label class="labelFont"><b>{{ wait_dataName }}</b></label></b-col>
      </b-row>
      <b-row class="my-1">
        <b-col sm="2"><label class="labelFont">Created Time: </label></b-col>
        <b-col sm="9"><label class="labelFont">{{ wait_createTime }}</label></b-col>
      </b-row>
      <b-row class="my-1">
        <b-col sm="2"><label class="labelFont">Task  Status: </label></b-col>
        <b-col sm="3">
          <label style="font-size: 20px; color: red">In check</label>
...
export default {
  ...
  methods: {
    ...
    upload () {
      this.wait_email = localStorage.getItem('email')
      this.wait_name = localStorage.getItem('name')
      ...
      if (sessionStorage['loginState']) {
        ...
        // this.$refs['modal-nofile'].show()
          ...
          const patBeta = /[a-zA-Z0-9_-]*_beta.csv/
          const patPh = /[a-zA-Z0-9_-]*_pheno.csv/
          if (patBeta.test(betaFile['name']) && patPh.test(phenoFile['name'])) {
            ...
            if (betaID === phenoID) {
              ...
              this.wait_id = sessionStorage['id']
              let dataName = betaFile['name'] + '  |  ' + phenoFile['name']
              this.wait_createTime = new Date(sessionStorage['id'] * 1000).toLocaleString().replace(/:\d{1,2}$/, ' ')
              sessionStorage.setItem('dataName', dataName)
              this.wait_dataName = sessionStorage['dataName']
              // this.$router.push({
              //   path: '/wait',
              //   name: 'Wait',
              //   params: {
              ...
              upload(formData, config).then(res => {
                if (res.data === 'unknownerror') {
                  this.errUnknownShow = true
                  this.dataUploading = false
                  return false
                }
