<template>
  <div class="browse-box" v-if="!readingDatabase">
    <b-container fluid>
      <b-row class="my-1" v-if="readingDatabase"><b-col><h3>We are reading database,please keep waiting</h3></b-col></b-row>
  ...
  <div class="browse-box" v-else> <b-button style="float: right" size="md" @click="cancelb()">cancel</b-button><b-row class="my-1"><b-col><h3>We are reading database,please keep this page opening</h3></b-col></b-row></div>
</template>
<script>
import {getDatasetsb, getDisease, getRace, getTissue, killpricess} from '../api'
...
export default {
  ...
  data () {
    return {
      ...
      sampleData: [], // 表格数据
      selectedrows: [{'name': 'test'},{'name': 'nothing'}],
      datasetsData: [],
      diseaseData: [],
      tissueData: [],
      raceData: [],
  ...
  methods: {
    cancelb () {
      ...
      formDatab.append('user_pid', 1234)
      killpricess(formDatab)
    },
    tbodyRowClass (item) {
  ...
  beforeDestroy () {
    console.log('destory')
    let formDatab = new FormData()
    formDatab.append('user_pid', 12345)
    killpricess(formDatab)
  },
  ...
  created () {
    ...
    const jsonData = JSON.stringify(this.selectedrows)
