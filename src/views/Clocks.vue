...
<template>
  <div class="clocksBox">
    ...
    <!--<b-pagination v-model="clocksCurrentPage" :total-rows="clocksTableRows" :per-page="clocksPerPage"
     ...
     <ul>
      <li v-for="clocksData in selected" :key="clocksData"><b>{{ clocksData.ClockName }}</b><hr></li>
    </ul>
      </div>
      <span>
...
export default {
  data () {
    return {
      ...
      clocksPerPage: 100,
      clocksCurrentPage: 1,
      fields: [{key:'Selected'},{key:'ClockName'}, {key:'FeaturesNum'}, {key:'LifePhase'}, {key:'Platform'}, {key:'Tissue'},{key: 'Reference'}],
      clocksData: [
        {
          id:1,
