<template>
  <div class="browse-box" v-if="!databaseReading">
    <b-container fluid>
      <b-row class="my-1" v-if="!readingDatabase"><b-col><h3>Select Data</h3></b-col></b-row>
      <hr />
      <b-row class="my-1">
        <!--选择模式-->
        <b-col sm="2" md="auto">
        ...
        <!--跳转结果页-->
        <b-col>
          <b-button style="float: right" size="md" @click="result" v-if="!resultreading">Result</b-button>
          <b-button style="float: right" size="md" v-if="resultreading">reading</b-button>
        </b-col>
      </b-row>
      <!--未选择数据集时 提示-->
...
<script>
import { getDatasets, getDisease, getRace, getTissue, read_datasets } from '../api'
export default {
  name: 'Browse',
  data () {
    return {
  ...
  methods: {
    ...
    result () {
      ...
      this.resultreading = true
      this.formDataValue = []
      const result = {}
      // result['geos'] = this.selectedGeoData
      result['clocks'] = this.selectedClocks
      if (this.selectedModel === 'datasets') {
        this.selectedRows.forEach(item => {
          this.formDataValue.push(item.Dataset)
        })
      }
      if (this.selectedModel === 'tissue') {
        this.selectedRows.forEach(item => {
          this.formDataValue.push(item.Tissues)
        })
      }
      if (this.selectedModel === 'race') {
        this.selectedRows.forEach(item => {
          this.formDataValue.push(item.Races)
        })
      }
      if (this.selectedModel === 'disease') {
        this.selectedRows.forEach(item => {
          this.formDataValue.push(item.Diseases)
        })
      }
      console.log(this.formDataValue)
      let formData = new FormData()
      formData.append('selected', this.formDataValue)
      formData.append('model', this.selectedModel)
      read_datasets(formData).then(res => {
  ...
  created () {
    getDatasets().then(res => {
      ...
      if (this.readingTissue === false && this.readingDatabase === false && this.readingRace === false && this.readingDisease === false) {
        this.databaseReading = false
      }
    })
    getDisease().then(res => {
      this.diseaseData = res.data['data']
      this.readingDisease = false
      if (this.readingTissue === false && this.readingDatabase === false && this.readingRace === false && this.readingDisease === false) {
        this.databaseReading = false
      }
    })
    getTissue().then(res => {
      ...
      this.readingTissue = false
      if (this.readingTissue === false && this.readingDatabase === false && this.readingRace === false && this.readingDisease === false) {
        this.databaseReading = false
      }
    })
    getRace().then(res => {
      this.readingRace = false
